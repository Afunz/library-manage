<template>
  <div style="padding: 20px">
    ...
    <div style="text-align: center; margin-top: 30px">
      <el-button type="primary" size="medium" @click="updateBookInfo"
        >修改</el-button
      >
...
<script>
import { getBookByIdAPI, updateBookInfoAPI, getCategoryListAPI } from "@/api";
  ...
  methods: {
    async updateBookInfo() {
      this.$refs["ruleForm"].validate(async (valid) => {
        if (valid) {
          const res = await updateBookInfoAPI(this.form);
          if (res.code === "200") {
