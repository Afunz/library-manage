...
<script>
import { getReturnInfoAPI, DeleteReturnByIdAPI, addReturnInfoAPI } from "@/api";
...
export default {
  ...
  methods: {
    async load() {
      const res = await getReturnInfoAPI(this.params);
      console.log(res);
      if (res.code === "200") {
    ...
    async delInfo(id) {
      const res = await DeleteReturnByIdAPI(id);
      if (res.code === "200") {
